...
Crime.prototype.addDateIndex = function(callback) {
  this.addIndex(getDateKey(cleanReportedDate(this.reported)), this.id, callback);
};

Crime.prototype.addBeatIndex = function(callback) {
  this.addIndex('BEAT:'+ this.beat, callback);
};
...
exports.getCrimesByDate = function(stem, callback) {
  client.keys(indexes.getIndexKey(getDateKey(stem)) + '*', function(dates) {
    if (dates.length > 0) {
      client.sunion.apply(client, dates, console.dir);
    }
    callback(dates);
  });
};
